...
vault policy write payment-evidence /vault/config/payment-evidence-policy.hcl >/dev/null

if ! vault auth list -format=json | jq -e 'has("approle/")' >/dev/null; then
  vault auth enable approle >/dev/null
fi

# Routine payment processing receives only encrypt and HMAC capabilities.
# Controlled decrypt is granted separately for the optional evidence workflow.
APP_POLICIES="payment-app"
case "${DEMO_EVIDENCE_ENABLED:-false}" in
  1|true|TRUE|yes|YES|on|ON) APP_POLICIES="payment-app,payment-evidence" ;;
esac

# The SecretID is rotated whenever the KMS container starts and expires after
# 24 hours. It remains file-delivered because this is a self-contained lab.
vault write auth/approle/role/payment-app \
  token_policies="$APP_POLICIES" \
  token_type=batch \
  token_ttl=15m \
  token_max_ttl=30m \
  token_num_uses=0 \
...
tmp_secret_id="/vault/app-credentials/.secret_id.new"
vault write -field=secret_id -f auth/approle/role/payment-app/secret-id \
  > "$tmp_secret_id"
mv "$tmp_secret_id" /vault/app-credentials/secret_id
...
unset VAULT_TOKEN ROOT_TOKEN UNSEAL_KEY APP_POLICIES
